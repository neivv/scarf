...
use crate::analysis;
use crate::disasm::{self, FlagArith, Disassembler32, DestOperand, FlagUpdate, Operation};
use crate::exec_state::{
    self, Constraint, FreezeOperation, Memory, MergeStateCache, PendingFlags, OperandCtxExtX86,
};
...
impl<'e> State<'e> {
        ...
        match *operation {
            ...
            Operation::Special(ref code) => {
                if &code[..] == &[0xd9, 0xf6] || &code[..] == &[0xd9, 0xf7] {
                    let mut fpu_state: [_; 8] = array_init::array_init(|i| {
                        self.resolve(ctx.register_fpu(i as u8))
                    });
                    if &code[..] == &[0xd9, 0xf6] {
                        // fdecstp
                        fpu_state.rotate_left(1);
                    } else {
                        // fincstp
                        fpu_state.rotate_right(1);
                    }
                    for i in 0..8 {
                        let mem = disasm::make_x86_fpu_mem_access(ctx, i);
                        let (base, offset) = mem.address();
                        self.write_memory(
                            base,
                            offset as u32,
                            mem.size,
                            fpu_state[i as usize],
                        );
                    }
                }
            }
    ...
    pub fn resolve(&mut self, value: Operand<'e>) -> Operand<'e> {
        ...
        if let Some(reg) = value.if_register() {
            return self.state[(reg & 7) as usize];
        }
        match *value.ty() {
            OperandType::Arch(arch) => {
                ...
                match tag {
                    ...
                    disasm::X86_REGISTER16_TAG | disasm::X86_REGISTER8_LOW_TAG =>
                    {
                        ...
                        if let Some(reg) = self.try_resolve_partial_register(reg, mask) {
                            return reg;
                        }
                        let value = ctx.and_const(self.resolve_register(reg), mask);
                        value
                    }
            ...
            OperandType::Arithmetic(ref op) => {
                ...
                if op.ty == ArithOpType::And {
                    if let Some(c) = right.if_constant() {
                        if let Some(left) = left.if_register() {
                            let r = self.try_resolve_partial_register(left, c);
                            if let Some(r) = r {
                                return r;
                            }
                        }
                ...
                if op.left.x86_if_flag().is_some() && left.relevant_bits().end > 1 {
