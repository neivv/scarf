    ...
    fn new() -> OperandCtxGlobals {
        OperandCtxGlobals {
            ...
            registers16: [
                ...
                Operand::new_simplified_rc(OperandType::Register16(Register(8))),
                Operand::new_simplified_rc(OperandType::Register16(Register(9))),
                Operand::new_simplified_rc(OperandType::Register16(Register(10))),
                Operand::new_simplified_rc(OperandType::Register16(Register(11))),
                Operand::new_simplified_rc(OperandType::Register16(Register(12))),
                Operand::new_simplified_rc(OperandType::Register16(Register(13))),
                Operand::new_simplified_rc(OperandType::Register16(Register(14))),
                Operand::new_simplified_rc(OperandType::Register16(Register(15))),
            ],
            registers8_low: [
                ...
                Operand::new_simplified_rc(OperandType::Register8Low(Register(8))),
                Operand::new_simplified_rc(OperandType::Register8Low(Register(9))),
                Operand::new_simplified_rc(OperandType::Register8Low(Register(10))),
                Operand::new_simplified_rc(OperandType::Register8Low(Register(11))),
                Operand::new_simplified_rc(OperandType::Register8Low(Register(12))),
                Operand::new_simplified_rc(OperandType::Register8Low(Register(13))),
            ...
            registers64: [
                ...
                Operand::new_simplified_rc(OperandType::Register64(Register(8))),
                Operand::new_simplified_rc(OperandType::Register64(Register(9))),
                Operand::new_simplified_rc(OperandType::Register64(Register(10))),
                Operand::new_simplified_rc(OperandType::Register64(Register(11))),
                Operand::new_simplified_rc(OperandType::Register64(Register(12))),
                Operand::new_simplified_rc(OperandType::Register64(Register(13))),
                Operand::new_simplified_rc(OperandType::Register64(Register(14))),
                Operand::new_simplified_rc(OperandType::Register64(Register(15))),
            ],
        }
    }
}
...
impl Operand {
    ...
    fn simplify_and_merge_child_ors(ops: &mut Vec<Rc<Operand>>, ctx: &OperandContext) {
        ...
        while let Some(mut op) = iter.next() {
            ...
            if let Some((mut constant, val)) = or_const(&op) {
                ...
                let constant = ctx.constant64(constant);
                let oper = if is_64 {
    ...
    // Simplify xor: merge regs
    // Converts ah | al => ax
    fn simplify_xor_merge_regs(ops: &mut Vec<Rc<Operand>>, ctx: &OperandContext) {
        ...
        while let Some(mut op) = iter.next() {
            let mut new = None;
            if let Some((is_high, r)) = is_reg8(&op) {
                let mut second = iter.duplicate();
                while let Some(other_op) = second.next_removable() {
                    if let Some((is_high2, r2)) = is_reg8(&other_op) {
                        if r == r2 && is_high != is_high2 {
    ...
    pub fn const_offset(oper: &Rc<Operand>, ctx: &OperandContext) -> Option<(Rc<Operand>, u64)> {
        ...
        if let Some(c) = oper.if_constant64() {
        ...
        fn recurse(oper: &Rc<Operand>) -> Option<u64> {
            ...
            match oper.ty {
                    ...
                    if arith.ty == ArithOpType::Add =>
                {
                    ...
                    } else if let Some(c) = arith.right.if_constant64() {
        ...
        if let Some(offset) = recurse(oper) {
            let base = Operand::simplified(operand_sub64(oper.clone(), ctx.constant64(offset)));
            Some((base, offset))
        } else {
            None
        }
    ...
    /// Returns `Some(r)` if `self.ty` is `OperandType::Register64(r)`
    pub fn if_register64(&self) -> Option<Register> {
        match self.ty {
            OperandType::Register64(r) => Some(r),
            _ => None,
        }
    }
...
fn simplify_mul(
    ...
    let const_product = ops.iter().flat_map(|x| x.if_constant64())
...
fn simplify_add_sub_ops(
    ...
    ops.retain(|&(ref x, _)| x.if_constant64().is_none());
    ops.sort();
    simplify_add_merge_muls(&mut ops, bit_size, ctx);
    if ops.is_empty() {
        if bit_size == 32 {
            return vec![(ctx.constant(const_sum as u32), false)];
        } else {
            return vec![(ctx.constant64(const_sum), false)];
        }
    }

    if bit_size == 32 {
        let const_sum = const_sum as u32;
        if const_sum != 0 {
        ...
        if const_sum != 0 {
            if const_sum > 0x8000_0000_0000_0000 {
                ops.push((ctx.constant64(0u64.wrapping_sub(const_sum)), true));
            } else {
                ops.push((ctx.constant64(const_sum), false));
            }
        }
    }
        ...
        if let (Some((mask1, l)), Some((mask2, r))) = (left_const, right_const) {
            ...
            if mask1 == mask2 && bit_size == 32 {
                ...
                if let Some((a, b, added_const)) = add_const {
                    let a = simplify_with_and_mask(a, mask1 as u64, ctx);
                    if a == *b {
...
fn simplify_and(
    ...
    loop {
        ...
        if const_remain != !0 {
            vec_filter_map(&mut ops, |op| {
                ...
                if let Some(c) = new.if_constant64() {
                    const_remain &= c;
                    None
                } else {
                    Some(new)
    ...
    if const_remain & relevant_bits != relevant_bits {
        ops.push(ctx.constant64(const_remain));
    }
        ...
        OperandType::Arithmetic(ref arith) | OperandType::Arithmetic64(ref arith) => {
            ...
            match arith.ty {
                ArithOpType::And => {
                    ...
                    if single_operand {
                        simplify_lsh(&tree, &right, bit_size, ctx, swzb_ctx)
                    } else {
                        let arith = ArithOperand {
                            ty: ArithOpType::Lsh,
                            left: tree,
                ...
                ArithOpType::Rsh => {
                    if let Some(rsh_const) = arith.right.if_constant() {
                        ...
                        simplify_and(&val, &ctx.constant64(mask), ctx, swzb_ctx)
                    } else {
                        default()
                    }
                }
...
fn simplify_rsh(
    ...
    match left.ty {
        ...
        OperandType::Constant64(a) => ctx.constant64(a >> constant),
        OperandType::Arithmetic(ref arith) | OperandType::Arithmetic64(ref arith) => {
            match left.ty {
                OperandType::Arithmetic(..) if bit_size == 32 => (),
                OperandType::Arithmetic64(..) if bit_size == 64 => (),
    ...
    match op.ty {
            ...
            match arith.ty {
                ...
                ArithOpType::Xor => {
                    ...
                    if simplified_left == arith.left && simplified_right == arith.right {
                        ...
                        let op = if is_64 {
                            operand_xor64(simplified_left, simplified_right)
                        } else {
                            operand_xor(simplified_left, simplified_right)
                        };
                        Operand::simplified(op)
                    }
                }
        ...
        OperandType::Memory(ref mem) => {
            ...
            let new_addr = if mask_low == 0 {
                ...
                operand_add64(mem.address.clone(), ctx.constant(mask_low))
            };
            ...
            let result = if offset_mask == nop_mask {
                ...
                operand_and(ctx.constant(offset_mask as u32), mem)
            };
            let shifted = if mask_low == 0 {
                result
            } else {
                operand_lsh64(result, ctx.constant(mask_low * 8))
            };
...
fn simplify_with_zero_bits(
    ...
    fn try_remove_const_and_mask<'a>(
        ...
        arith.and_then(|(arith, bit_size)| {
            Operand::either(&arith.left, &arith.right, |x| {
                x.if_constant().and_then(|constant| {
                    // Check if the constant has only single block of one bits
                    let rel_bits = x.relevant_bits();
                    let low = rel_bits.start;
                    let high = bit_size - rel_bits.end.min(bit_size);
                    let mask = !0 >> low << low << high >> high;
                    if constant == mask {
                        Some(())
                    } else {
            ...
            .and_then(|((), other)| {
                ...
                // since `bits` are zero.
                let can_remove_const = bits.end >= bit_size &&
                    other_bits.end < bit_size &&
    ...
    match op.ty {
                    ...
                    if new_left.is_some() || new_right.is_some() {
                        ...
                        return if is_64 {
                            Some(Operand::simplified(
                                operand_add64(new_left.clone(), new_right.clone())
                            ))
                        } else {
                            Some(simplify_add_sub(new_left, new_right, bit_size, false, ctx))
                        };
                    }
                }
                _ => (),
        ...
        OperandType::Register64(r) | OperandType::Register(r) | OperandType::Register16(r) => {
            if bits.end == 64 {
                if bits.start <= 8 {
                    return Some(ctx.register8_low(r.0));
                } else if bits.start <= 16 && relevant_bits.end > 16 {
                    return Some(ctx.register16(r.0));
...
fn simplify_with_one_bits(
    ...
    match op.ty {
                ...
                ArithOpType::And => {
                    ...
                    match (left, right) {
                        ...
                        (None, Some(s)) | (Some(s), None) => {
                            ...
                            let high = 64 - bits.end;
                            let mask = !0u64 >> low << low << high >> high;
                            Some(Operand::simplified(operand_and64(ctx.constant64(mask), s)))
                        }
        ...
        OperandType::Constant(c) => {
            let low = bits.start;
            let high = 64 - bits.end;
            let mask = (!0u64 >> low << low << high >> high) as u32;
            let new_val = c | mask;
            match new_val & !mask {
                0 => None,
                c => Some(ctx.constant(c)),
            ...
            Some(Some((sum, equiv))) => {
                ...
                if bit_size == 32 {
                    ...
                    if sum > 0x8000_0000_0000_0000 {
                        let sum = !sum.wrapping_add(1);
                        ops[pos].0 = Operand::simplified(operand_mul64(ctx.constant64(sum), equiv));
                        ops[pos].1 = true;
                    } else {
                        ops[pos].0 = Operand::simplified(operand_mul64(ctx.constant64(sum), equiv));
                        ops[pos].1 = false;
...
pub mod operand_helpers {
    ...
    pub fn operand_register64(num: u8) -> Rc<Operand> {
        OperandContext::new().register64(num)
    }
...
mod test {
    ...
    fn or_64() {
        ...
        let op1 = Operand::simplified(operand_or64(
            constval64(0xffff_0000_0000),
            constval64(0x12456),
        ));
        let eq1 = constval64(0xffff_0001_2456);
        let op2 = Operand::simplified(operand_or64(
            operand_register64(0),
            constval(0),
        ));
        let eq2 = operand_register64(0);
        assert_eq!(Operand::simplified(op1), Operand::simplified(eq1));
        assert_eq!(Operand::simplified(op2), Operand::simplified(eq2));
    }
